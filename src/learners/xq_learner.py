...
class XQLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None):
        ...
        if self.args.mixer == "xmix":
            ...
            mac_z = mac_z.reshape(batch.batch_size, self.args.n_agents, batch.max_seq_length, -1).transpose(1,2) 
        mac_hidden_states = th.stack(mac_hidden_states, dim=1)
        mac_hidden_states = mac_hidden_states.reshape(batch.batch_size, self.args.n_agents, batch.max_seq_length, -1).transpose(1,2) 
        ...
        with th.no_grad():
            ...
            if self.args.mixer == "xmix":
                target_max_qvals, _ = self.target_mixer(target_max_qvals, mac_hidden_states, batch["state"], batch["obs"])
            else:
        ...
        if self.args.mixer == "xmix":
            chosen_action_qvals, temporal_credits = self.mixer(chosen_action_qvals, mac_hidden_states[:,:-1], batch["state"][:, :-1], batch["obs"][:, :-1])
